...
export default function HostedClaimPage({ params }: { params: { brandSlug: string } }) {
  ...
  if (success) {
    return (
      <div className="bg-grid-mesh flex min-h-screen items-center justify-center px-4">
        <div className="w-full max-w-md rounded-2xl border border-border/60 bg-card p-8 text-center shadow-floating">
          ...
          <h1 className="mt-5 text-xl font-semibold tracking-tight">Claim submitted</h1>
          <p className="mt-2 text-sm text-muted-foreground">Reference: <span className="font-mono text-foreground">{success}</span></p>
  ...
  return (
    <div className="bg-grid-mesh flex min-h-screen flex-col items-center justify-center px-4 py-12">
      <div className="w-full max-w-lg rounded-2xl border border-border/60 bg-card p-8 shadow-floating">
        <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-xl bg-primary/10 text-primary">
          <ShieldCheck className="h-6 w-6" />
        </div>
        <p className="mt-4 text-center text-sm font-medium text-muted-foreground">{brand.name}</p>
        <h1 className="mt-1 text-center text-2xl font-semibold tracking-tight">Submit warranty claim</h1>
        <form onSubmit={(e) => void onSubmit(e)} className="mt-8 space-y-4">
          ...
          <div className="space-y-2">
            ...
            <textarea
              className="min-h-24 w-full rounded-lg border border-input bg-background px-3 py-2 text-sm shadow-xs-soft focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/50"
              value={description}
